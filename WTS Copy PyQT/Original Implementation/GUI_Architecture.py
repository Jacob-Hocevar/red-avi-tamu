...
class GUI_Root():
    def __init__(self):
        ...
        # RED logo
        self.RED_logo = Image.open("G:/Shared drives/TAMU Rocket Engine Design/02 Subsystems - General/04 AVI - Avionics & Controls/03 WTS/WTS_GUI/RED_logo.png")
...
class GUI_COM():
    def __init__(self, root, serial):
        ...
        self.port_label = Label(self.frame, text="COM Port: ", width=20, anchor="w", bg="#000", fg="#fff")
        self.baud_label = Label(self.frame, text="Baud Rate: ", width=20, anchor="w", bg="#000", fg="#fff")
    ...
    def get_port_options(self):
        ...
        self.drop_ports.config(width=12, bg="#000", fg="#fff", highlightbackground="black", highlightcolor="grey")
    ...
    def get_baud_options(self):
        ...
        self.drop_bauds.config(width=12, bg="#000", fg="#fff", highlightbackground="black", highlightcolor="grey")
    ...
    def connect_click(self):
        ...
        if self.btn_connect["text"] in "Connect":
            ...
            if self.serial.ser.status:
                ...
                # Update size
                self.root.geometry("1050x427") # update size
...
class GUI_CTRL():
    def __init__(self, root, serial):
        ...
        # valva labels
        self.val1_label = Label(self.frame, text="VALVE A Control: ", width=21, anchor="w", bg="#000", fg="#fff")
        self.val2_label = Label(self.frame, text="VALVE B Control: ", width=21, anchor="w", bg="#000", fg="#fff")
        # control state label
        self.ctrl_label = Label(self.frame, text="Control State: ", width=21, anchor="w", bg="#000", fg="#fff")
        self.ctrl_state = Label(self.frame, text="Operation", width=10, anchor="center", bg="#000", fg="#fff")
        ...
        self.btn_val2_on = Button(self.frame, text="OPEN", width=6, bg="#000", fg="#fff", highlightbackground="#000", highlightcolor="#fff", state="disable", command=self.v2_OPEN)
        ...
        self.btn_val2_off = Button(self.frame, text="CLOSE", width=6, bg="#000", fg="#fff", highlightbackground="#000", highlightcolor="#fff", state="normal", command=self.v2_CLOSE)
        ...
        # self.btn_open = Button(self.frame, text="OPEN", width=16, bg="#000", fg="#fff", highlightbackground="#000", highlightcolor="#fff", state="normal", command=self.ctrl_open)
    ...
    def display(self):
        ...
        if self.manual:
            ...
            self.ctrl_label.grid(column=0,row=3, pady=5)
            self.ctrl_state.grid(column=1,row=3, columnspan=2)
            # on/off buttons
            ...
            # self.btn_open.grid(column=1,row=4,padx=5, pady=5)
    ...
    def v1_OPEN(self):
        ...
        if self.v2 == 1:
            self.ctrl_state.config(text="Fully Open")
        else:
            self.ctrl_state.config(text="Pressurization")
    ...
    def v2_OPEN(self):
        ...
        if self.v1 == 1:
            ...
            self.ctrl_state.config(text="Operation")
    ...
    def v1_CLOSE(self):
        ...
        if self.v2 == 0:
            self.ctrl_state.config(text="Fully Closed")
        else:
            self.ctrl_state.config(text="Operation")
    ...
    # control state buttons commands
    # def ctrl_off(self):
    #     '''
    ...
    #     if self.recording:
    #         self.record_valve_states("Open whole system")
    ...
    def ctrl_save_start(self):
        ...
        '''       
        try:
            filename = "G:/Shared drives/TAMU Rocket Engine Design/02 Subsystems - General/04 AVI - Avionics & Controls/03 WTS/WTS_GUI/Data/" + str(time.time_ns()) + "_control_data.csv"
            self.control_data = open(filename, 'w')
            self.recording = True

            # Header and initial state
            self.control_data.write("Global Time (ns from Epoch),Valve A State (1=Open),Valve B State(1=Open),Last Command\n")
...
class GUI_DAQ():
    def __init__(self, root, serial, port, control):
        ...
        self.RHO = 998.0                          # kg/m^3
         # No orifice
        #self.A = 3.14159/4.0*(0.401*0.0254)**2
        #self.Cd = 0.64165
        ...
        # Tank volume
        self.vol_start = 5.0*0.0037854118        # m^3, (initial volume in gal)*( 1 m^3/ 1 gal)
    ...
    def read_data(self):
        ...
        try:
            ...
            local_time = int(local_time) / 1000000.0           # teeensy gives microseconds
            self.pt1 = float(p1)
            self.pt2 = float(p2)
            self.calculate_mfr()
        except:
            #print(Read too many values (or other error)")
            return 
    ...
    def daq_save_start(self):
        ...
        '''
        try:
            filename = "G:/Shared drives/TAMU Rocket Engine Design/02 Subsystems - General/04 AVI - Avionics & Controls/03 WTS/WTS_GUI/Data/" + str(time.time_ns()) + "_pressure_data.csv"
...
class GUI_GRAPH():
    def __init__(self, root, serial, daq):
        ...
        self.max_psi = 164.7 + 14.754

        # Plot layout
        self.fig, self.axs = plt.subplots(1, 2)
        self.fig.set_figheight(3.55)
    ...
    def update_graphs(self, frame_data):
        ...
        ax.grid()
        if len(self.x_vals):
            bottom = min(min(self.daq.plt_pt1), min(self.daq.plt_pt2))
            if bottom < 0:
                bottom*=1.1
            else:
                bottom*=0.9
            top = max(max(self.daq.plt_pt1), max(self.daq.plt_pt2))
            if top < 0:
                top*=0.9
            else:
                top*=1.1
